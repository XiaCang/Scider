...
describe('library API', () => {
  ...
  describe('fetchLibraryApi', () => {
    it('应发送 GET /papers', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: [{ id: 'p-1', title: 'Paper' }] } })
  ...
  describe('fetchPaperByIdApi', () => {
    it('应发送 GET /papers/{id}', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: { id: 'p-1', title: 'Test' } } })
  ...
  describe('saveKeyPointsApi', () => {
    it('应发送 PATCH /papers/{id}/key-points', async () => {
      ...
      request.patch.mockResolvedValue({ data: { code: 0 } })
  ...
  describe('deletePaperApi', () => {
    it('应发送 DELETE /papers/{id}', async () => {
      ...
      request.delete.mockResolvedValue({ data: { code: 0 } })
  ...
  describe('folder CRUD', () => {
    it('fetchFoldersApi — GET /folders/', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: [{ id: 'f-1', name: 'ML' }] } })
      const result = await fetchFoldersApi()
      expect(request.get).toHaveBeenCalledWith('/folders/')
      expect(result.data.data[0].name).toBe('ML')
    })

    it('createFolderApi — POST /folders/', async () => {
      const { createFolderApi } = await import('./library')
      request.post.mockResolvedValue({ data: { code: 0, data: { id: 'f-2', name: 'New' } } })
    ...
    it('fetchFolderDetailApi — GET /folders/{id}', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: { id: 'f-1' } } })
    ...
    it('createSubFolderApi — POST /folders/{id}/subfolders', async () => {
      ...
      request.post.mockResolvedValue({ id: 'f-1-1', name: 'Sub' })
    ...
    it('copyFolderApi — POST /folders/{id}/copy', async () => {
      ...
      request.post.mockResolvedValue({ id: 'f-copy', name: 'Copy' })
  ...
  describe('paper-folder关联', () => {
    ...
    it('fetchFolderPapersApi — GET /folders/{id}/papers', async () => {
      ...
      request.get.mockResolvedValue([{ id: 'p-1' }])
  ...
  describe('PDF预览', () => {
    it('fetchPaperPdfInfoApi — GET /papers/{id}/pdf-info', async () => {
      ...
      request.get.mockResolvedValue({ id: 'p-1', title: 'Test', pdfUrl: '/pdfs/test.pdf', pageCount: 5 })
  ...
  describe('笔记操作', () => {
    it('fetchPaperNotesApi — GET /notes/', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: { total: 1, items: [{ id: 'n-1' }] } } })
    ...
    it('createNoteApi — POST /notes/', async () => {
      ...
      request.post.mockResolvedValue({ data: { code: 0, data: { id: 'n-1' } } })
    ...
    it('fetchNoteDetailApi — GET /notes/{id}', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: { id: 'n-1', title: 'Note' } } })
    ...
    it('updateNoteApi — PATCH /notes/{id}', async () => {
      ...
      request.patch.mockResolvedValue({ data: { code: 0, data: { id: 'n-1' } } })
    ...
    it('uploadNoteImageApi — POST /notes/uploads 含 FormData', async () => {
      ...
      request.post.mockResolvedValue({ data: { code: 0, data: { url: '/uploads/img.png' } } })
    ...
    it('fetchNoteImagesApi — GET /notes/{id}/images', async () => {
      ...
      request.get.mockResolvedValue({ data: { code: 0, data: { total: 1, items: [{ id: 'img-1' }] } } })
  ...
  describe('PDF上传', () => {
    it('uploadPaperApi — POST /papers/upload 含 FormData', async () => {
      ...
      request.post.mockResolvedValue({ data: { code: 0, data: { paper_id: 'p-1', filename: 'paper.pdf', file_size: 3, md5: 'abc', status: 'ok', task_id: 't-1' } } })
    ...
    it('batchUploadPapersApi — POST /papers/upload 多文件', async () => {
      ...
      request.post.mockResolvedValue({ data: { code: 0, data: { total: 2, success_count: 2, results: [] } } })
  ...
  describe('PDF搜索', () => {
    it('searchInPaperApi — POST /papers/{id}/search', async () => {
      ...
      request.post.mockResolvedValue({ data: { code: 0, data: { keyword: 'transformer', total_results: 2, results: [{ page_number: 1, content: 'transformer model', score: 0.9, highlights: ['transformer'] }] } } })
